...
export const Page = () => {
  ...
  const [footer, footerState] = useSinglePrismicDocument("footer", uid);


  const notFound = pageState.state === "failed" || menuState.state === "failed" || footerState.state === "failed";
  ...
  if (page && menu) {
    return (
      <Layout wrapperClass="page" menuDoc={menu} footerDoc={footer}>
